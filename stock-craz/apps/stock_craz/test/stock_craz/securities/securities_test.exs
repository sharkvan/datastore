  ...
  describe "dividend_declarations" do
    ...
    def dividend_declaration_fixture(attrs \\ %{}) do
      {:ok, dividend_declaration} =
        ...
        |> Securities.create_dividend_declaration()
    ...
    test "list_dividend_declarations/0 returns all dividend_declarations" do
      ...
      assert Securities.list_dividend_declarations() == [dividend_declaration]
    end
    ...
    test "create_dividend_declaration/1 with valid data creates a dividend_declaration" do
      assert {:ok, %DividendDeclaration{} = dividend_declaration} = Securities.create_dividend_declaration(@valid_attrs)
      ...
      assert dividend_declaration.stock_id == 42
    end

    test "create_dividend_declaration/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Securities.create_dividend_declaration(@invalid_attrs)
    end

    test "update_dividend_declaration/2 with valid data updates the dividend_declaration" do
      dividend_declaration = dividend_declaration_fixture()
      assert {:ok, dividend_declaration} = Securities.update_dividend_declaration(dividend_declaration, @update_attrs)
      ...
      assert dividend_declaration.stock_id == 43
    end

    test "update_dividend_declaration/2 with invalid data returns error changeset" do
      dividend_declaration = dividend_declaration_fixture()
      assert {:error, %Ecto.Changeset{}} = Securities.update_dividend_declaration(dividend_declaration, @invalid_attrs)
