defmodule WebUi.DividendDeclarationControllerTest do
  ...
  def fixture(:dividend_declaration) do
    {:ok, dividend_declaration} = Securities.create_dividend_declaration(@create_attrs)
    dividend_declaration
  end

  describe "index" do
    test "lists all dividend_declarations", %{conn: conn} do
      conn = get conn, dividend_declaration_path(conn, :index)
  ...
  describe "new dividend_declaration" do
    test "renders form", %{conn: conn} do
      conn = get conn, dividend_declaration_path(conn, :new)
  ...
  describe "create dividend_declaration" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, dividend_declaration_path(conn, :create), dividend_declaration: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == dividend_declaration_path(conn, :show, id)

      conn = get conn, dividend_declaration_path(conn, :show, id)
      assert html_response(conn, 200) =~ "Show Dividend declaration"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, dividend_declaration_path(conn, :create), dividend_declaration: @invalid_attrs
      assert html_response(conn, 200) =~ "New Dividend declaration"
    end
  end
  ...
  describe "edit dividend_declaration" do
    ...
    test "renders form for editing chosen dividend_declaration", %{conn: conn, dividend_declaration: dividend_declaration} do
      conn = get conn, dividend_declaration_path(conn, :edit, dividend_declaration)
  ...
  describe "update dividend_declaration" do
    ...
    test "redirects when data is valid", %{conn: conn, dividend_declaration: dividend_declaration} do
      conn = put conn, dividend_declaration_path(conn, :update, dividend_declaration), dividend_declaration: @update_attrs
      assert redirected_to(conn) == dividend_declaration_path(conn, :show, dividend_declaration)

      conn = get conn, dividend_declaration_path(conn, :show, dividend_declaration)
      assert html_response(conn, 200)
    end

    test "renders errors when data is invalid", %{conn: conn, dividend_declaration: dividend_declaration} do
      conn = put conn, dividend_declaration_path(conn, :update, dividend_declaration), dividend_declaration: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Dividend declaration"
    end
  end
  ...
  describe "delete dividend_declaration" do
    ...
    test "deletes chosen dividend_declaration", %{conn: conn, dividend_declaration: dividend_declaration} do
      conn = delete conn, dividend_declaration_path(conn, :delete, dividend_declaration)
      assert redirected_to(conn) == dividend_declaration_path(conn, :index)
      assert_error_sent 404, fn ->
        get conn, dividend_declaration_path(conn, :show, dividend_declaration)
      end
    end
  end
